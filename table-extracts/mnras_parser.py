...
def search_and_add_mnras_footnote_to_obj(footnotes, data, json_obj):
    ...
    for footnote in footnotes:
        ...
        footnotes[footnote] = footnotes[footnote].replace(
            'Note.', '').replace('Notes.', '').strip()
    ...
    for footnote in valid_footnotes:
        if footnote in data:
            ...
            json_obj['note'] = valid_footnotes[footnote].replace(
                footnote, '').replace('Notes.', '').replace('Note.', '').strip()
...
def search_mnras_table_info_and_footnotes(table):
    ...
    if notes_elements:
        ...
        for notes_element in notes_elements:
            ...
            footnote_tags = notes_element.find_all(
                "div", {"class": "footnote"})
            if footnote_tags:
